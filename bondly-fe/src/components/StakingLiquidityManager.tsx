...
const StakingLiquidityManager: React.FC<StakingLiquidityManagerProps> = ({ isMobile }) => {
    ...
    onLogs: (logs) => {
      ...
      const approvalLog = logs.find(log => {
        ...
        return args?.owner?.toLowerCase() === address?.toLowerCase() &&
               args?.spender?.toLowerCase() === stakingAddress.toLowerCase();
      });
  ...
  return (
    <div style={{
      ...
      >
        {isLoading || isPending ? 'Processing...' : 'Add BOND Staking Liquidity'}
      </button>
