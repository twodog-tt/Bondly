...
const mockBlogPosts: BlogPost[] = [
  {
    ...
    content: {
      ...
      summary:
        "Uniswap V4 引入了革命性的 Hook 机制，本文将详细分析其技术实现和潜在影响...",
      body: "Uniswap V4 的 Hook 机制是一个重大创新，它允许开发者在流动性池的生命周期中注入自定义逻辑。",
      images: [
        "https://via.placeholder.com/600x300/667eea/ffffff?text=Uniswap+V4+Hook",
      ],
      codeBlocks: [
        {
          language: "solidity",
  ...
  },
  {
    ...
    content: {
      ...
      summary:
        "随着 Web3 生态的快速发展，钱包安全变得尤为重要。本文总结了最新的安全实践...",
...
export default function Feed({ isMobile }: FeedProps) {
  ...
  const allTags = Array.from(
    new Set(mockBlogPosts.flatMap((post) => post.metadata.tags)),
  );
  ...
  const animatedBlogPostStyle = (index: number) => ({
    ...
    transform: visiblePosts.includes(index)
      ? "translateY(0)"
      : "translateY(30px)",
        ...
        {loading
          ? // 骨架屏
            [...Array(3)].map((_, index) => (
              <div
                key={index}
                style={isMobile ? mobileBlogPostStyle : blogPostStyle}
              >
                <div
                  style={{
                    height: "200px",
            ...
            filteredPosts.map((post, index) => (
              <div key={post.id}>
                <div
                  style={
                    isMobile
                      ? animatedMobileBlogPostStyle(index)
                      : animatedBlogPostStyle(index)
                  }
                  onMouseEnter={(e) => {
                ...
                >
                  ...
                  <div style={postSummaryStyle}>{post.content.summary}</div>
                  ...
                  {/* 内容预览 */}
                  <div style={contentPreviewStyle}>{post.content.body}...</div>
                  ...
                  {/* 代码块展示 */}
                  {post.content.codeBlocks &&
                    post.content.codeBlocks.length > 0 && (
                  ...
                  <div style={postActionsStyle}>
                    ...
                    <div style={statsStyle}>
                      <div style={statItemStyle}>👁️ {post.stats.views}</div>
                      <div style={statItemStyle}>📤 {post.stats.shares}</div>
                    </div>
                  </div>
                </div>
