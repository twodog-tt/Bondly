...
export default function Editor({ isMobile }: EditorProps) {
  ...
  // 处理媒体插入
  const handleMediaInsert = (mediaUrl: string, mediaType: 'image' | 'video' | 'audio') => {
    const mediaMarkdown = mediaType === 'image' 
      ? `![图片](${mediaUrl})`
      : mediaType === 'video'
    ...
    <div style={containerStyle}>
      ...
      <div style={editorContainerStyle}>
        ...
        <div style={editorPanelStyle}>
          ...
          <textarea
            placeholder={editorMode === 'markdown' ? "使用Markdown语法编写文章内容..." : "请输入文章内容..."}
            value={content}
          ...
          <div style={statusBarStyle}>
            ...
            />
            <div>
              字符数: {content.length} | 字数: {content.replace(/\s/g, '').length}
            </div>
          </div>
          ...
          <div style={{ display: 'flex', gap: '12px', justifyContent: 'flex-end', marginTop: '16px' }}>
            ...
            <button style={primaryButtonStyle} onClick={() => setShowPublishModal(true)}>
              🚀 发布文章
            </button>
          </div>
        </div>
