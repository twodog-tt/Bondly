...
const highlights = [
  ...
  { icon: '⚙️', label: 'architecture_design', value: 'architecture_design_desc' },
];

const roadmap = [
  { stage: 'stage_one', content: 'progressive_login', status: 'completed' },
  { stage: 'stage_two', content: 'content_publishing', status: 'completed' },
  { stage: 'stage_three', content: 'reputation_calculation', status: 'in_progress' },
...
export default function Home({ isMobile }: HomeProps) {
  ...
  const handleCreateUserSubmit = async () => {
    try {
      ...
      */
      ...
      // 显示成功消息
      const avatarInfo = avatarChoice === 'upload' && avatar ? `\n头像: ${avatar.name}` : '\n头像: 未上传';
  ...
  const getStepDescription = () => {
    switch (step) {
      ...
      case 2:
        return isCodeSent ? t('verification_code_step_desc') : t('email_step_desc');
      case 3:
    ...
    <div style={isMobile ? mobileContainerStyle : containerStyle}>
      <div style={isMobile ? mobileHeroStyle : heroStyle}>
        ...
        <div style={heroContent}>
          ...
          <p style={isMobile ? mobileSubtitleStyle : subtitleStyle}>{t('decentralized_social_value_network')}</p>
          <p style={{ 
            fontSize: isMobile ? '16px' : '18px', 
            color: 'rgba(255,255,255,0.8)' 
          }}>{t('slogan')}</p>
          <button 
            style={isMobile ? mobileCreateUserButtonStyle : createUserButtonStyle}
            onClick={(e) => {
              createRipple(e);
              handleCreateUser();
            }}
      ...
      <div style={isMobile ? mobileSectionStyle : sectionStyle}>
        <div style={isMobile ? mobileSectionDecoration : sectionDecoration}></div>
        <div style={sectionContent}>
          <h2 style={isMobile ? mobileSectionTitleStyle : sectionTitleStyle}>
            📋 {t('project_intro')}
          </h2>
          <p style={{ fontSize: isMobile ? '16px' : '18px', lineHeight: '1.8', color: '#4a5568' }}>
      ...
      <div style={isMobile ? mobileSectionStyle : sectionStyle}>
        ...
        <div style={sectionContent}>
          ...
          <div style={isMobile ? mobileHighlightsGridStyle : highlightsGridStyle}>
            {highlights.map((item, index) => (
              ...
              >
                <div style={isMobile ? mobileCardDecoration : cardDecoration}></div>
                <div style={cardContent}>
                  <div style={isMobile ? mobileHighlightLabelStyle : highlightLabelStyle}>
                    {item.icon} {t(item.label)}
                  </div>
                  <div style={isMobile ? mobileHighlightValueStyle : highlightValueStyle}>
                    {t(item.value)}
                  </div>
                </div>
              </div>
      ...
      <div style={isMobile ? mobileSectionStyle : sectionStyle}>
        ...
        <div style={sectionContent}>
          ...
          <div style={{ fontSize: isMobile ? '16px' : '18px', lineHeight: '1.8', color: '#4a5568' }}>
      ...
      <div style={isMobile ? mobileSectionStyle : sectionStyle}>
        ...
        <div style={sectionContent}>
          ...
          {roadmap.map((item) => (
            ...
            >
              <span style={isMobile ? mobileStageStyle : stageStyle}>{t(item.stage)}</span>
              <span style={isMobile ? mobileContentStyle : contentStyle}>{t(item.content)}</span>
              <span style={isMobile ? mobileStatusStyle : statusStyle}>{item.status === 'completed' ? '✅' : item.status === 'in_progress' ? '⏳' : '🔜'}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      {showModal && (
        <div style={modalOverlayStyle} onClick={handleCloseModal}>
          <div style={isMobile ? mobileModalStyle : modalStyle} onClick={(e) => e.stopPropagation()}>
            <div style={stepIndicatorStyle}>
              <div style={step === 1 ? activeStepDotStyle : stepDotStyle}></div>
              <div style={step === 2 ? activeStepDotStyle : stepDotStyle}></div>
              <div style={step === 2 && isCodeSent ? completedStepDotStyle : stepDotStyle}></div>
            ...
            {step === 1 && (
              <>
                <input
                  ...
                  style={usernameError ? inputErrorStyle : username.trim() && !usernameError ? inputSuccessStyle : inputStyle}
                  autoFocus
                />
                {usernameError && (
            ...
            {step === 2 && (
              <>
                <input
                  ...
                  onChange={handleInputChange}
                  onKeyPress={handleKeyPress}
                  style={emailError ? inputErrorStyle : email.trim() && !emailError ? inputSuccessStyle : inputStyle}
                  autoFocus
                />
                {emailError && (
                  <div style={errorMessageStyle}>
                  ...
                  <>
                    <div style={verificationContainerStyle}>
                      <input
                        ...
                        onKeyPress={handleKeyPress}
                        style={verificationCodeError ? verificationInputErrorStyle : verificationInputStyle}
                        maxLength={6}
                        autoFocus
                      />
                      <button
                        style={isVerifying ? verifyButtonDisabledStyle : verifyButtonStyle}
                        onClick={handleVerifyCode}
                        disabled={isVerifying || !verificationCode.trim() || !!verificationCodeError}
                        onMouseEnter={(e) => {
                          if (!isVerifying && verificationCode.trim() && !verificationCodeError) {
            ...
            {step === 3 && (
              <div style={avatarUploadStyle}>
                {/* 头像选择选项 */}
                <div style={isMobile ? mobileAvatarOptionsStyle : avatarOptionsStyle}>
                  <button 
                    style={avatarChoice === 'upload' ? selectedAvatarOptionStyle : avatarOptionButtonStyle}
                  ...
                  <button 
                    style={avatarChoice === 'skip' ? selectedAvatarOptionStyle : avatarOptionButtonStyle}
                    ...
                    {avatarPreview ? (
                      <div style={{ position: 'relative' as const }}>
                        <img 
                          src={avatarPreview} 
                          alt="Avatar Preview" 
                          style={isMobile ? mobileAvatarPreviewStyle : avatarPreviewStyle}
                          onClick={() => {
                      ...
                      <div 
                        style={isMobile ? mobileAvatarUploadAreaStyle : avatarUploadAreaStyle}
                        onClick={() => {
                          const input = document.createElement('input');
                          input.type = 'file';
                          input.accept = 'image/*';
                ...
                {avatarChoice === 'skip' && (
                  <div style={{
                    ...
                    <p style={{ margin: '0 0 8px 0', fontSize: '16px' }}>✅ 已选择跳过头像</p>
