...
func main() {
	...
	go func() {
		if err := srv.Start(); err != nil {
			loggerpkg.Log.Fatalf("Failed to start server: %v", err)
		}
	}()
	...
	loggerpkg.Log.Info("Shutting down server...")
	...
	if err := srv.Shutdown(ctx); err != nil {
		loggerpkg.Log.Errorf("Server forced to shutdown: %v", err)
	}

	loggerpkg.Log.Info("Server exited")
}
